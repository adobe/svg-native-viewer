...
void GIDPlusSVGPath::Rect(float x, float y, float width, float height) 
{ 
    mPath.AddRectangle(Gdiplus::RectF(x, y, width, height));
}

void GIDPlusSVGPath::RoundedRect(float x, float y, float w, float h, float r)
{
    const float d = r + r;
...
void GIDPlusSVGPath::Ellipse(float cx, float cy, float rx, float ry)
{
    mPath.AddEllipse(cx - rx, cy - ry, rx + rx, ry + ry);
}

void GIDPlusSVGPath::MoveTo(float x, float y)
{
    mCurrentX = x;
    mCurrentY = y;
}

void GIDPlusSVGPath::LineTo(float x, float y)
{
...
void GIDPlusSVGPath::CurveTo(float x1, float y1, float x2, float y2, float x3, float y3)
{
...
void GIDPlusSVGPath::CurveToV(float x2, float y2, float x3, float y3)
{
...
void GIDPlusSVGPath::ClosePath() 
{ 
    mPath.CloseFigure();
}

const Gdiplus::GraphicsPath& GIDPlusSVGPath::GetGraphicsPath() const
{
    return mPath;
}
...
GIDPlusSVGTransform::GIDPlusSVGTransform(float a, float b, float c, float d, float tx, float ty) 
{ 
    mTransform.SetElements(a, b, c, d, tx, ty);
}

void GIDPlusSVGTransform::Set(float a, float b, float c, float d, float tx, float ty) 
{ 
    mTransform.SetElements(a, b, c, d, tx, ty);
}

void GIDPlusSVGTransform::Rotate(float r) 
{ 
    mTransform.Rotate(r);
}

void GIDPlusSVGTransform::Translate(float tx, float ty) 
{ 
    mTransform.Translate(tx, ty);
}

void GIDPlusSVGTransform::Scale(float sx, float sy) 
{ 
    mTransform.Scale(sx, sy);
}

void GIDPlusSVGTransform::Concat(const Transform& other)
{
    mTransform.Multiply(&dynamic_cast<const GIDPlusSVGTransform&>(other).mTransform);
}

const Gdiplus::Matrix& GIDPlusSVGTransform::GetMatrix() const
{
    return mTransform;
}

GIDPlusSVGImageData::GIDPlusSVGImageData(const std::string& base64, ImageEncoding encoding)
{
...
GIDPlusSVGImageData::~GIDPlusSVGImageData()
{
}

float GIDPlusSVGImageData::Width() const
{
    return static_cast<float>(mImage->GetWidth());
}

float GIDPlusSVGImageData::Height() const
{
    return static_cast<float>(mImage->GetHeight());
}

const std::unique_ptr<Gdiplus::Image>& GIDPlusSVGImageData::GetImage() const
{
    return mImage;
}

GIDPlusSVGRenderer::GIDPlusSVGRenderer() 
{
}

void GIDPlusSVGRenderer::Save(const GraphicStyle& graphicStyle)
{
    mStateStack.push_back(mContext->Save());

    if (graphicStyle.transform)
    {
        const Gdiplus::Matrix* matrix = &dynamic_cast<GIDPlusSVGTransform*>(graphicStyle.transform.get())->GetMatrix();
        if (matrix != nullptr)
        {
...
void GIDPlusSVGRenderer::Restore()
{
    mContext->Restore(mStateStack.back());
    mStateStack.pop_back();
}

std::unique_ptr<Gdiplus::Brush> GIDPlusSVGRenderer::CreateGradientBrush(const Gradient& gradient, float opacity)
{
    ...
    GIDPlusSVGTransform* gdiplusSvgTransform = dynamic_cast<GIDPlusSVGTransform*>(gradient.transform.get());
...
void GIDPlusSVGRenderer::DrawPath(const Path& renderPath, const GraphicStyle& graphicStyle, const FillStyle& fillStyle, const StrokeStyle& strokeStyle)
{
    SVG_ASSERT(mContext);
    Gdiplus::GraphicsState savedState = mContext->Save();

    std::unique_ptr<Gdiplus::GraphicsPath> path(dynamic_cast<const GIDPlusSVGPath&>(renderPath).GetGraphicsPath().Clone());
...
void GIDPlusSVGRenderer::DrawImage(const ImageData& image, const GraphicStyle& graphicStyle, const Rect& clipArea, const Rect& fillArea)
{
    const GIDPlusSVGImageData& gdiPlusImage(dynamic_cast<const GIDPlusSVGImageData&>(image));
